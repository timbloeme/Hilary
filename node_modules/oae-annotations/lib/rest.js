...
OAE.tenantServer.get('/api/annotations/:revisionId/:page/:annotationId', function(req, response){
    ...
    Cassandra.runQuery('SELECT ? FROM Annotations WHERE revisionId = ?', [annotationId, req.params.revisionId], function(err, rows){
        if (err) {
            return response.send(err.code, err.msg);
        } else if (rows[0].count <= 0) {
            return response.send(200, []);
...
OAE.tenantServer.post('/api/annotations/:revisionId/:page/:user', function(req, response){
    // Generate the newId and append it to the data we already have.
    var newId = req.params.page + '_' + Shortid.generate();
    var data = req.body;
    data.id = newId;
    data.user = req.params.user;
    Cassandra.runQuery('UPDATE Annotations SET ? = ? WHERE revisionId = ?', [newId, JSON.stringify(data), req.params.revisionId], function(err){
        if (err) {
            return response.send(err.code, err.msg);
        }
...
// Update a annotation
OAE.tenantServer.put('/api/annotations/:revisionId/:page/:user/:annotationId', function(req, response){
